<template>
  <div class="min-h-screen bg-[#2c1e14] text-[#e0d3b8] p-4 md:p-6 font-sans">
    ...
    <div v-else class="max-w-[1500px] mx-auto space-y-4">

      <!-- 空狀態 -->
      <div v-if="accounts.length === 0 && !isAdding"
           class="text-center py-20 text-[#a6937c] italic bg-[#3d2b1f] rounded-xl border border-[#5e4b37] text-base">
      ...
      <div v-if="isAdding"
        ...
        <div class="bg-[#2c1e14] px-5 py-4 flex items-center gap-3 border-b border-[#5e4b37]">
          <span class="text-[#4a7c59] text-sm font-bold bg-[#1a3326] px-3 py-1 rounded">新增</span>
        ...
        <div class="p-5 space-y-3">
          ...
          <div v-for="(role, rIdx) in newAccount.roles" :key="rIdx"
            ...
            <div class="col-span-4">
              <select v-model="role.job"
                      class="w-full bg-[#2c1e14] border border-[#5e4b37] rounded px-3 py-2 text-base text-[#e0d3b8] outline-none focus:border-[#f1d483] cursor-pointer transition">
          ...
          <button @click="addRoleRow(newAccount.roles)"
                  class="text-sm text-[#a6937c] hover:text-[#f1d483] transition mt-1">＋ 新增角色</button>
      ...
      <div v-for="(acc, aIdx) in accounts" :key="acc.name"
        ...
        <!-- 帳號標頭：整列可點擊展開（非編輯模式） -->
        <div class="bg-[#2c1e14] px-5 py-4 flex items-center justify-between border-b border-[#5e4b37]"
          ...
          <div class="flex items-center gap-2 shrink-0 ml-3" @click.stop>
            <!-- 編輯模式操作 -->
            <template v-if="editingIdx === aIdx">
              ...
              <button @click="cancelEdit"
                      class="text-sm text-[#a6937c] hover:text-[#e0d3b8] transition">取消</button>
            </template>
            <!-- 一般模式操作 -->
            <template v-else>
...
const loadAccounts = async () => {
  ...
  try {
    ...
    accounts.value.forEach((_, i) => { collapsed.value[i] = false; }); // 預設展開
