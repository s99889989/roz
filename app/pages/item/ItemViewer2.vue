...
// 假設您的物品數據放在這裡。
// ***注意：這裡需要您提供完整的 items_database.js 文件。
// 為了程式碼能運行，我將暫時用您提供的藥水和模擬的飾品數據來構造一個 itemsData。***
const sampleItemsData = itemsData;

const items = ref(sampleItemsData); // 這裡使用模擬數據
...
const subcategories = computed(() => {
  ...
  const allSubcategories = Object.values(items.value)
      .filter(item => item.category.startsWith(selectedCategory.value))
      .map(item => item.category.split('/')[1]) // 提取斜線後的子類別
      .filter(sub => sub); // 過濾掉沒有子類別的項目
...
const filteredItemsArray = computed(() => {
  ...
  if (selectedSubcategory.value && selectedSubcategory.value !== 'ALL') {
    ...
    itemsArray = itemsArray.filter(item => item.category === fullSubcategory);
  }
...
const getWearingImg = (id) => new URL(`/assets/images/wearing/${id}.gif`, import.meta.url).href;
</script>
    ...
    <div class="content-card" style="width: 100%; max-width: 1000px; padding: 15px 25px;">
      <div class="top-controls">
        <input
            ...
            placeholder="請輸入物品名稱"
            class="search-input"
            style="width: 250px; margin: 0; display: inline-block; vertical-align: middle;"
        >
        <button @click="clearFilters" class="clear-button" style="float: right; margin-top: 5px;">清除篩選條件</button>
      </div>
      ...
      <div class="subcategory-panel" style="margin-top: 20px;">
      ...
      <div class="item-card-grid" style="display: flex; flex-wrap: wrap; gap: 20px; justify-content: flex-start;">
        ...
        >
          <div style="display: flex; justify-content: space-between; margin-bottom: 10px;">
            ...
            <span class="card-type-tab" style="padding: 3px 8px; background-color: #6a5a4d; border-radius: 4px;">{{ item.category.split('/')[1] || '-' }}</span>
          </div>
          <div class="flex w-full justify-center">
            <img :src="getItemImg(item.id)" alt="" class="h-8">
            <img :src="getWearingImg(item.id)" alt="" class="h-8">
          </div>
          ...
          <p style="font-size: 0.9em; margin-bottom: 15px; height: 75px; overflow: hidden; color: #c9c1b6; line-height: 1.5;">
