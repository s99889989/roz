    ...
    <div class="max-w-[1400px] mx-auto">
      ...
      <div v-else>
        ...
        <div v-if="totalCount > 0" class="text-sm text-[#a6937c] mb-3">
          共 <span class="text-[#f1d483] font-bold">{{ totalCount }}</span> 筆結果
                ...
                <tr v-for="(item, i) in shopResults" :key="i"
                  ...
                  <td class="px-4 py-2.5">
                    <button @click="openDetail(item)"
                            class="text-[#f1d483] hover:underline font-bold text-left">
                      {{ item.storeName }}
                    </button>
                  </td>
        ...
        <!-- 查無資料 -->
        <div v-if="totalCount === 0 && shopResults.length === 0 && historyResults.length === 0 && keyword"
    ...
    <Transition name="modal">
      <div v-if="showLogin" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4"
        ...
        <div class="bg-[#2c1e14] border border-[#5e4b37] rounded-xl p-6 w-full max-w-sm shadow-2xl">
          ...
          <div v-if="loginLoading" class="mb-3 text-sm text-[#a6937c] bg-[#3d2b1f] border border-[#5e4b37] rounded px-3 py-2 text-center">
            ⏳ 正在開啟瀏覽器並自動登入，請稍候...
          </div>
