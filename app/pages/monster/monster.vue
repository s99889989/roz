...
<template>
  ...
  <Transition name="fade">
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" @click.self="closeModal">
      ...
      <div class="relative w-full max-w-md bg-[#4a4540] rounded-lg border border-[#6b6359] overflow-hidden shadow-2xl animate-scale-up">
        ...
        <span class="bg-[#5a5550] px-3 py-1 rounded text-sm text-gray-200 shadow-inner">
          ...
          <button @click="closeModal" class="bg-[#5a5550] hover:bg-red-900 text-gray-300 w-8 h-8 rounded flex items-center justify-center transition-colors">
            ✕
          </button>
        </div>
        ...
        <div class="p-6 text-center">
          ...
          <div class="grid grid-cols-2 gap-y-2 gap-x-8 text-left text-gray-300 text-sm border-t border-[#6b6359] pt-4">
            <div class="flex justify-between border-b border-[#5a5550] pb-1">
              <span class="text-gray-500">需求等級</span> <span>{{ selectedFullItem.attributes?.required_level || 1 }}</span>
            </div>
            <div class="flex justify-between border-b border-[#5a5550] pb-1">
              <span class="text-gray-500">重量</span> <span>{{ selectedFullItem.attributes?.weight || 0 }}</span>
            </div>
            <div class="flex justify-between border-b border-[#5a5550] pb-1">
              <span class="text-gray-500">買價</span> <span>{{ selectedFullItem.buy_price?.toLocaleString() || 'N/A' }}</span>
            </div>
            <div class="flex justify-between border-b border-[#5a5550] pb-1">
              <span class="text-gray-500">賣價</span> <span>{{ selectedFullItem.sell_price?.toLocaleString() || 'N/A' }}</span>
            </div>
            <div class="flex justify-between border-b border-[#5a5550] pb-1">
        ...
        <div class="bg-[#3a3530] p-4 border-t border-[#6b6359]">
          ...
          <div class="flex flex-wrap gap-4">
            <div v-for="source in selectedFullItem.dropped_by" :key="source.monster_id" class="flex items-center text-sm text-gray-300">
