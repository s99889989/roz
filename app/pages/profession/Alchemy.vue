<template>
  <div class="p-6 bg-[#2B1E12] text-[#4b2e1e] rounded min-h-screen">
    <h1 class="text-3xl font-bold mb-6 text-[#FFC80A]">武器製作與藥水製作總表（分章節、可折疊）</h1>
      ...
      <transition name="fade">
        <div v-show="opened.potions" class="mt-3 bg-[#fffaf0] p-4 rounded border">
          ...
          <table class="w-full border-collapse">
            <thead class="bg-[#f0e0c3]"><tr><th class="p-2 border">藥水</th><th class="p-2 border">材料</th></tr></thead>
            <tbody>
            <tr v-for="r in other" :key="r.name"><td class="p-2 border">{{ r.name }}</td><td class="p-2 border">{{ r.materials.join(' + ') }}</td></tr>
            </tbody>
          </table>
...
<!--    </footer>-->
    ...
    <!-- 新增：煉金AI生命體區塊（依照你的樣式整合在底部） -->
    <section class="mt-10 bg-[#3a2c1f] p-6 rounded">
      <h2 class="text-2xl font-bold text-[#c9a063] text-center mb-4">煉金AI生命體</h2>
      <p class="text-center mb-4">以下列出各種 AI 生命體相關資源與下載連結</p>

      <div class="overflow-x-auto border border-[#8b5a2b] rounded">
        <table class="w-full border-collapse bg-[#3A2C1F] text-[#fff8e1] min-w-[650px]">
          <thead>
          <tr class="bg-[#5b4835] text-[#c9a063] font-bold">
            <td class="p-3 border border-[#5b4835] w-[10%]">順序</td>
            <td class="p-3 border border-[#5b4835] w-[40%] text-left">名稱</td>
            <td class="p-3 border border-[#5b4835] w-[40%]">連結</td>
          </tr>
          </thead>
          <tbody>
          <tr v-for="(item, idx) in contentList" :key="item.id" :class="{ 'bg-[#423528]': idx % 2 === 1 }" class="hover:bg-[#503d2b] transition">
            <td class="p-3 border border-[#5b4835] text-center">{{ item.id }}</td>
            <td class="p-3 border border-[#5b4835] text-left font-bold text-[#ffcc66]">{{ item.title }}</td>
            <td class="p-3 border border-[#5b4835] text-center">
              <template v-if="item.link && item.link !== '#'">
                <a :href="item.link" target="_blank" class="inline-flex items-center bg-[#c9a063] text-[#3A2C1F] font-bold px-3 py-2 rounded hover:bg-[#e6c589] transition">
                  查看詳情 →
                </a>
              </template>
              <template v-else>
                <span class="inline-block px-3 py-2 bg-[#503d2b] text-[#aaa] border border-[#666] rounded">尚無資料</span>
              </template>
            </td>
          </tr>
          </tbody>
...
const other = ref([
  { name: '酒精', materials: ['植物梗 × 5', '毒魔菇芽孢 × 5', '空瓶 × 1', '空試管 × 1'] },
  { name: '火煙瓶', materials: ['透明之布 × 1', '酒精 × 1', '空瓶 × 1'] },
  { name: '鹽酸瓶', materials: ['不死心臟 × 1', '空瓶 × 1'] },
  { name: '植物瓶', materials: ['噬人樹花 × 2', '空瓶 × 1'] },
  { name: '鏈鎚瓶', materials: ['腦管 × 1', '血管 × 1', '空瓶 × 1'] },
  { name: '護貝藥', materials: ['酒精 × 1', '茲諾克之牙 × 1', '人魚之心 × 1', '空瓶 × 1'] },
]);
